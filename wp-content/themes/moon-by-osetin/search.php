...
<div class="<?php echo esc_attr($content_left_css_class); ?>" style="<?php echo esc_attr($content_left_style); ?>">
  ...
  <div class="content-left-i activate-perfect-scrollbar">
    <div class="content-self" style="<?php echo esc_attr($content_left_self_style); ?>">
      <div><h1><?php printf( __( '"%s" <span class="smaller-text">Query Search Results</span>', 'moon' ), ucwords(get_search_query()) ); ?></h1></div>
      ...
      <div class="desc"><?php
        ...
        if(!empty($categories)){
          echo '<h5 class="spacer">'.__('Some Categories:').'</h5>';
          echo '<ul class="list-in-content-left">';
          foreach ($categories as $category) {
            echo '<li><a href="'.get_category_link( $category->term_id ).'" title="' . sprintf( __( "View all posts in %s", "moon" ), $category->name ) . '">'.$category->name.' <span>( '.$category->count.' )</span></a></li>';
          }
          echo '</ul>';
        }
        ?>
  ...
  <div class="content-right-i activate-perfect-scrollbar">
      ...
      <?php if ( have_posts() ){ ?>
        ...
        <div class="no-results">
          ...
          <h3><span><?php _e('No results found', 'moon'); ?></span></h3>
          <p><?php _e('Make sure all words are spelled correctly and try again:', 'moon') ?></p>
