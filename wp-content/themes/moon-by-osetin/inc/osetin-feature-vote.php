...
function osetin_vote_build_button($post_id, $extra_class = '', $heart_icon_class = 'os-icon-heart', $liked = false, $like = false, $label_first = false){
  if($liked == false) $liked = __("Liked", "moon");
  if($like == false) $like = __("Like", "moon");
  $vote_data = osetin_vote_get_post_vote_data($post_id);
  ?>
  <a href="#" 
      class="<?php echo esc_attr($extra_class); ?> osetin-vote-trigger <?php echo ($vote_data['has_voted']) ? 'osetin-vote-has-voted' : 'osetin-vote-not-voted'; ?>" 
      data-has-voted-label="<?php echo esc_attr($liked); ?>" 
      data-not-voted-label="<?php echo esc_attr($like); ?>" 
      data-post-id="<?php echo esc_attr($post_id); ?>" 
      data-vote-action="<?php echo ($vote_data['has_voted']) ? 'unvote' : 'vote'; ?>" 
      data-votes-count="<?php echo esc_attr($vote_data['count']); ?>">
    <span class="slide-button-i">
      <?php if($label_first == false) echo '<i class="os-icon '.$heart_icon_class.'"></i>'; ?>
      <span class="slide-button-label osetin-vote-action-label tile-button-label">
        <?php echo ($vote_data['has_voted']) ? $liked : $like; ?>
      </span>
      <span class="slide-button-sub-label osetin-vote-count <?php if(!$vote_data['count']) echo 'hidden'; ?>">
        <?php echo $vote_data['count'];  ?>
      </span>
      <?php if($label_first) echo '<i class="os-icon '.$heart_icon_class.'"></i>'; ?>
    </span>
  </a><?php
}
