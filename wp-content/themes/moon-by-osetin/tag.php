...
<div class="<?php echo esc_attr($content_left_css_class); ?>" style="<?php echo esc_attr($content_left_style); ?>">
  ...
  <div class="content-left-i activate-perfect-scrollbar">
    <div class="content-self" style="<?php echo esc_attr($content_left_self_style); ?>">
      <div><h1><?php printf( __( '%s <span class="smaller-text">Tag Archives</span>', 'moon' ), ucwords(single_tag_title( '', false )) ); ?></h1></div>
      ...
      <div class="desc"><?php
        ...
        <?php } ?>
        <h5 class="spacer">Other Tags:</h5>
        <?php 
        $tags = get_tags(); 
        if(!empty($tags)){
          echo '<ul class="list-in-content-left">';
          foreach ($tags as $tag) {
            echo '<li><a href="'.get_tag_link( $tag->term_id ).'" title="' . sprintf( __( "View all posts in %s", "moon" ), $tag->name ) . '">'.$tag->name.' <span>( '.$tag->count.' )</span></a></li>';
          }
          echo '</ul>';
        }
        ?>
